...
<template>
    <section class="login">
        <div class="login__tabs" role="tablist" aria-label="Переключение формы авторизации">
            <!-- Табы для переключения формы авторизации -->
            <button type="button" class="login__tab-btn" :class="{ 'is-active': authTab === 'login' }" role="tab"
                :aria-selected="authTab === 'login'" @click="switchTab('login')">
                Вход
            </button>
            <button type="button" class="login__tab-btn" :class="{ 'is-active': authTab === 'register' }" role="tab"
                :aria-selected="authTab === 'register'" @click="switchTab('register')">
                Регистрация
            </button>
        </div>
        ...
        <form class="login__form" novalidate @submit.prevent="submitAuth"> <!-- Форма для входа -->
            <AppInput v-model="userEmail" label="Email" placeholder="xxxxxx@xxx.xx" type="email" autocomplete="email"
                :error="emailError" />
            <AppInput v-model="userPassword" label="Password" placeholder="xxxxxx" type="password"
                :autocomplete="authTab === 'login' ? 'current-password' : 'new-password'" :error="passwordError" />
            <AppInput v-if="authTab === 'register'" v-model="userPasswordConfirm" label="Repeat password"
                placeholder="xxxxxx" type="password" autocomplete="new-password" :error="passwordConfirmError" />

            <AppButton class="login__btn" type="submit" variant="secondary" :loading="isSubmitting">
                {{ submitButtonText }}
            </AppButton>
