...
# useValidation:
Почему соблюдены требования: 
- вход — произвольная форма из любых полей и словарь правил по полям.
- любое число полей и правил
- проверка поля и всей формы (validateField, validateAll) 
- реактивность :общая валидность (isFormValid) и по каждому полю (fieldStates / errors)
- правила расширяемы, в т.ч. асинхронные.

# useApiRequest:
Почему соблюдены требования: 
- в опции передаются URL, метод, заголовки, тело, query-параметры
- запрос через HTTP (axios)
- в ответе реактивны: данные (data), код ответа (status)
- флаги жизненного цикла (isLoading, isSuccess, isError) и текст ошибки (error), плюс execute для вызова.

![Требования](Frontend/assets/readme/requirements.png)
